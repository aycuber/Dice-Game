...
def make_averaged(original_function, samples_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def actual_averaged(*num_rolls):
        total = 0.0
        for i in range(samples_count):
            total += original_function(*num_rolls)
        return total / samples_count
    return actual_averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    ans = 1
    for i in range (10):
        temp = make_averaged(roll_dice,samples_count)
        temp2 = temp(i+1, dice)
        if temp2 > max:
            max = temp2
            ans = i+1
